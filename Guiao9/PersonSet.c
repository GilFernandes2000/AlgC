...
PersonSet* PersonSetCreate() {
  ...
  set->size = 0;
  set->array = (Person**)malloc(set->capacity*sizeof(Person));
  if (set == NULL) {
     perror("PersonCreate"); exit(2); 
  }
  set->size++;
  return set;
}
...
int PersonSetEquals(const PersonSet *ps1, const PersonSet *ps2) {
  ...
  if(PersonSetIsSubset(ps1, ps2) && ps1->size == ps2->size) return 1;
  return 0;
}
