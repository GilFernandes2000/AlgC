// NMEC: 93460
// NOME: João Gil Ferreira de Sousa Fernandes
//
...
static int _treeGetHeightREC(const struct _BSTreeNode* root) {
 if (root == NULL)
  ...
  int lheight = _treeGetHeightREC(root->left);
  int rheight = _treeGetHeightREC(root->right);
...
void* BSTreeGetMin(const BSTree* header) {
  ...
  struct _BSTreeNode *currNode = header->root->left;
...
void* BSTreeSearch(const BSTree* header, const void* item) {
  ...
  while (current != NULL) {
    int side = header->compare(current->item, item);
    if(side > 0){
...
static void _BSTreeAddItems(const struct _BSTreeNode* p, Queue* q) {
  ...
  return _BSTreeAddItems(p->left,q);
}
// Q: What kind of tree traversal is this function doing?
// 1) Breadth-first,
// 2) Depth-first pre-order,
